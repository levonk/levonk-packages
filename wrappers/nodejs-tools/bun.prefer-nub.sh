...
echo "⚠️ Prefer nub npm pnpm yarn over bun. Detecting available alternatives..."

# List available alternatives
available_found=false
for tool in nub npm pnpm yarn; do
...
if [ "$available_found" = true ]; then
    echo "💡 Tip: Multiple alternatives detected. Consider standardizing on nub for best compatibility."
fi
