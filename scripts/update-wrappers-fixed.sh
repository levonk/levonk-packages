...
update_wrapper() {
    ...
    # Get the first preferred tool
    local first_tool=$(echo "$preferred_tools" | awk '{print $1}')
...
if [ "\$available_found" = true ]; then
    echo "💡 Tip: Multiple alternatives detected. Consider standardizing on $first_tool for best compatibility."
fi

# Fallback to preferred tool
echo "🔄 Running $first_tool..."
exec "$first_tool" "\$@"
EOF
            ;;
        force)
            cat > "$wrapper_file" << EOF
#!/usr/bin/env sh
echo "✅ Using $first_tool instead of $target_binary (forced by policy)..."
exec "$first_tool" "\$@"
EOF
            ;;
        block)
